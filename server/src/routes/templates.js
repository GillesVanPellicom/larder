...
router.get('/', async (_req, res) => {
    try {
        ...
        const result = templateRows.map((t) => formatTemplate(t, versionMap.get(t.currentVersionId)));
        res.json(result);
    }
    catch (err) {
    ...
    try {
        ...
        for (const fieldId of fieldIds) {
         ...
         WHERE template_id = $1 
           ...
           AND (field_values->>$2) != '{}'
         LIMIT 10`, [id, fieldId]);
            const countResult = await pool.query(`SELECT COUNT(*)::int as count FROM recipes 
         WHERE template_id = $1 
           AND deleted_at IS NULL
           AND field_values ? $2 
           AND (field_values->>$2) IS NOT NULL 
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const prevFields = latestVersion?.fieldsSchema || [];
        ...
        if (removedFieldIds.length > 0 && !body.archiveRemovedFields && !body.confirmPurgeRemovedFields) {
            ...
            for (const fid of removedFieldIds) {
                const countRes = await pool.query(`SELECT COUNT(*)::int as count FROM recipes 
           WHERE template_id = $1 
             AND deleted_at IS NULL
             AND field_values ? $2 
             AND (field_values->>$2) IS NOT NULL 
             AND (field_values->>$2) != '' 
             AND (field_values->>$2) != '[]' 
             AND (field_values->>$2) != '{}'`, [id, fid]);
                const count = countRes.rows[0]?.count || 0;
                if (count > 0) {
                    const sampleRes = await pool.query(`SELECT title FROM recipes 
             WHERE template_id = $1 
               AND deleted_at IS NULL
               AND field_values ? $2 
             LIMIT 5`, [id, fid]);
        ...
        if (body.isDefault !== undefined)
            updatePayload.isDefault = Boolean(body.isDefault);
